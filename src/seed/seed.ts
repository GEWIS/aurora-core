...
export async function seedOpeningSequence(
  ...
  const movingHeadsWhite: StaticColorCreateParams = {
    type: 'StaticColor', props: { color: RgbColor.WHITE },
  };
  ...
  const registerDim = async (
    ...
    await Promise.all(new Array(Math.round(steps) + 1).fill(0).map(async (x, i) => {
      ...
      }, dimStart + i * stepDuration, stepDuration + 25, [room]);
      await addStep({
        ...effectBar,
        props: { ...effectBar.props, relativeBrightness },
      }, dimStart + i * stepDuration, stepDuration + 25, [bar]);
    }));
  };
  ...
  await addStep({ type: 'SearchLight', props: { radiusFactor: 1.5 } }, 28000, 189000, [movingHeadsGEWIS]);

  // Refrein 1
  await addStep(movingHeadsWhite, 28000, 32000, [movingHeadsGEWIS]);
  await addStep(allOfTheLights, 27500, 1500, [room, bar]);
  await addStep({ type: 'Wave', props: { color: RgbColor.ROSERED } } as WaveCreateParams, 29000, 4000, spots);
  await addStep(allOfTheLights, 33000, 2000, [room, bar]);
  await addStep({ type: 'Wave', props: { color: RgbColor.ROSERED } } as WaveCreateParams, 35000, 5000, spots);
  await addStep(allOfTheLights, 40000, 1500, [room, bar]);
  await addStep({ type: 'Wave', props: { color: RgbColor.ROSERED } } as WaveCreateParams, 41500, 13500, spots);

  // Couplet
  await addStep({ type: 'BeatFadeOut', props: { colors: [RgbColor.PURPLE, RgbColor.GREEN], nrBlacks: 1 } }, 55000, 5500, spots);
  // BLACKOUT (at 1 minute exactly for 2,5 seconds)
  await addStep({ type: 'BeatFadeOut', props: { colors: [RgbColor.PURPLE, RgbColor.GREEN], nrBlacks: 1 } }, 62500, 19000, spots);
  ...
  await addStep({ type: 'Sparkle', props: { colors: [RgbColor.GREEN, RgbColor.BROWN] } }, 96000, 12400, spots);
  await addStep(allOfTheLights, 108400, 1600, spots);
  await addStep({ type: 'BeatFadeOut', props: { colors: [RgbColor.ORANGE, RgbColor.LIGHTBLUE], nrBlacks: 1 } }, 110000, 25800, spots);
  ...
  await addStep({ type: 'Sparkle', props: { colors: [RgbColor.BLUE, RgbColor.LIME] } }, 150000, 12500, spots);
  await addStep(allOfTheLights, 162500, 1000, spots);
  await addStep({
    type: 'Sparkle',
    props: {
      colors: [RgbColor.GOLD], dimDuration: 2000, cycleTime: 125, ratio: 0.05,
    },
  }, 163500, 27000, spots);
  await addStep({ type: 'Wave', props: { color: RgbColor.YELLOW, cycleTime: 425 } }, 190500, 27500, spots);

  await addStep(movingHeadsWhite, 218000, 26000, [movingHeadsGEWIS]);
  await addStep({ type: 'SearchLight', props: { cycleTime: 10000 } }, 218000, 26000, [movingHeadsGEWIS]);
  ...
  await addStep({ type: 'BeatFadeOut', props: { colors: [RgbColor.RED, RgbColor.CYAN] } }, 262000, 23000, spots);
  await addStep(movingHeadsWhite, 262000, 23000, [movingHeadsGEWIS]);
  await addStep({ type: 'RandomPosition', props: { beatsToMove: 1 } }, 262000, 23000, [movingHeadsGEWIS]);
