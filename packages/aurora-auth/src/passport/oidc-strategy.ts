...
import { parseRoles } from '@gewis/aurora-core/helpers/security';
...
passport.use(
  ...
  new CustomStrategy(async (req, callback) => {
    if (!req.body.state || !req.body.session_state || !req.body.code) {
      resolveLogger().error('Missing OIDC state, session state or code.');
    ...
    if (
      ...
      resolveLogger().error('Not all OIDC environment variables are properly defined');
      callback(new HttpApiException(HttpStatusCode.InternalServerError));
      return;
    }
    ...
    if (!oidcConfig) {
      resolveLogger().error('Failed to fetch the OIDC configuration endpoint.');
      callback(new HttpApiException(HttpStatusCode.InternalServerError));
      return;
    }
